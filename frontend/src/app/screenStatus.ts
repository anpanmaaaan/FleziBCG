...
export const SCREEN_STATUS_REGISTRY: Record<string, ScreenStatusEntry> = {
  ...
  stationExecution: {
    ...
    dataSource: "BACKEND_API",
    notes:
      "Execution actions and session workflow (open, identify operator, bind equipment, close) are connected to backend APIs under the session-owned execution model.",
  },
  qcCheckpoints: {
    routePattern: "/quality",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to quality gate definition list/create APIs for backend-owned checkpoint configuration truth.",
  },
  defectManagement: {
    routePattern: "/defects",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to quality nonconformance list/create APIs. Backend remains source of truth for defect/nonconformance lifecycle.",
  },
  traceability: {
  ...
  userManagement: {
    ...
    phase: "PARTIAL",
    dataSource: "BACKEND_API",
    notes: "User list is live from backend IAM. Activate/deactivate lifecycle actions are connected. Create user and role/scope assignment require additional workflows.",
  },
  roleManagement: {
    routePattern: "/roles",
    phase: "SHELL",
  ...
  securityEvents: {
    ...
    phase: "PARTIAL",
    dataSource: "BACKEND_API",
    notes: "Security events are live from the backend IAM system. Backend remains source of truth for threat detection and incident response.",
  },
  tenantSettings: {
  ...
  operationTimeline: {
    ...
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to work order execution timeline API. Shows operation-level timing data for the work order containing the requested operation.",
  },
  stationSession: {
    routePattern: "/station-session",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Station session viewer is connected to GET /station/sessions/current. Shows live session state, operator, and equipment. Close session action is connected.",
  },
  operatorIdentification: {
    routePattern: "/operator-identification",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Operator identification is backend-connected through station session identify-operator and current-session endpoints. Backend remains execution and authorization truth.",
  },
  equipmentBinding: {
    routePattern: "/equipment-binding",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Equipment binding is connected to station session current + bind-equipment endpoint. Requires stationId query param.",
  },
  lineMonitor: {
  ...
  qualityDashboard: {
    ...
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Real-time aggregation of quality holds, deviation requests, and nonconformances with KPI cards. Reads from backend APIs; write actions route to individual detail pages.",
  },
  measurementEntry: {
    routePattern: "/quality-measurements",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to quality measurement requirements and submit APIs. Frontend pre-loads requirement items and limits; backend validates and derives quality/quantity effects.",
  },
  qualityHolds: {
    routePattern: "/quality-holds",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to quality hold list and disposition APIs. Supports RELEASE_QC_HOLD, ACCEPT_WITH_DEVIATION, REQUIRE_RECHECK, and CONFIRM_SCRAP disposition codes.",
  },
  qualityDeviations: {
    routePattern: "/quality-deviations",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to deviation request list and resolve APIs. Backend is source of truth for resolution status.",
  },
  qualityNonconformances: {
    routePattern: "/quality-nonconformances",
    phase: "CONNECTED",
    dataSource: "BACKEND_API",
    notes: "Connected to nonconformance list and create APIs. Backend is source of truth for NC status and disposition.",
  },
  materialReadiness: {
