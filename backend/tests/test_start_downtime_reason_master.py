...
# ─── 9. schema rejects missing or blank reason_code ─────────────────────────
def test_start_downtime_request_rejects_missing_reason_code():
    """
    `reason_code` is the only valid reason input. The request schema must
    reject missing or blank values before reaching the service. This is
    what makes the service-layer `INVALID_REASON_CODE` fallback unreachable
    ...
    with _pytest.raises(ValidationError):
        OperationStartDowntimeRequest()  # type: ignore[call-arg]

    with _pytest.raises(ValidationError):
        OperationStartDowntimeRequest(reason_code="")
