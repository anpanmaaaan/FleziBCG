...
def test_alembic_head_is_baseline():
    ...
    Updated: 0009 (drop_station_claims) is now head;
    ...
    -> 0008 (boms) -> 0009 (drop_station_claims).
    This test validates the migration chain is linear and has a single head.
    """
    script_dir = _get_script_dir()
    heads = script_dir.get_heads()
    assert len(heads) == 1, f"Expected exactly one head, got: {heads}"
    assert "0009" in heads, f"Expected 0009 as head, got: {heads}"
...
def test_alembic_upgrade_head_live(db_engine):
    ...
    assert "0009" in rows, f"Expected 0009 in alembic_version, got: {rows}"
