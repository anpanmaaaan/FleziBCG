...
def _derive_cycle_time_minutes(actual_start: datetime | None, actual_end: datetime | None) -> int | None:
...
def _derive_planned_cycle_minutes(planned_start: datetime | None, planned_end: datetime | None) -> int | None:
...
def _derive_cycle_time_delta(operation) -> int | None:
    cycle_minutes = _derive_cycle_time_minutes(operation.actual_start, operation.actual_end)
    planned_cycle_minutes = _derive_planned_cycle_minutes(operation.planned_start, operation.planned_end)
...
def build_work_order_operation_summaries(db: Session, work_order_id: int) -> list[OperationListItem]:
    ...
    for operation in operations:
        ...
        delayed_count = sum(1 for value in historical_delay_minutes if value is not None and value > 0)
        ...
        historical_cycle_deltas = [value for value in historical_cycle_deltas if value is not None]

        cycle_time_delta = _derive_cycle_time_delta(operation)
        cycle_time_minutes = _derive_cycle_time_minutes(operation.actual_start, operation.actual_end)
        qc_fail_count = sum(1 for item in operation_history if (item.scrap_qty or 0) > 0)
    ...
    wo_blocked_operations = sum(1 for _, _, bucket, *_ in operation_projections if bucket == "BLOCKED")
    wo_delayed_operations = sum(1 for _, _, bucket, *_ in operation_projections if bucket == "DELAYED")
    ...
    return [
        OperationListItem(
            ...
            route_step=operation.work_order.production_order.route_id if operation.work_order and operation.work_order.production_order else None,
        )
        for (
            operation,
            delay_minutes,
