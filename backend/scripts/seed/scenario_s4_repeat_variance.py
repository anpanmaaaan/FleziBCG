...
def seed(db: Session) -> list[ScenarioContext]:
    ...
    historical_windows = [
        ...
        ("2020-03-11T08:00:00", "2020-03-11T09:00:00", 60),   # 60 minute cycle (variance!)
        ("2020-03-12T08:00:00", "2020-03-12T10:00:00", 120),  # 120 minute cycle (high variance!)
    ]

    for index, (wo_start, wo_end, cycle_minutes) in enumerate(historical_windows, start=1):
    ...
    run_start(db, first_op.id, started_at=datetime.fromisoformat(first_op.planned_start.isoformat()))
    # Complete with 15-minute cycle (falls within variance of historical data)
    completion = datetime.fromisoformat(first_op.planned_start.isoformat()) + timedelta(minutes=15)
