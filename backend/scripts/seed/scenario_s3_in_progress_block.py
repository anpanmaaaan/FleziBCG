...
from .common import (
    ...
    mark_blocked_for_incident_seed,
    run_start,
)
...
def seed(db: Session) -> ScenarioContext:
    ...
    run_start(
        db, first_op.id, started_at=first_op.planned_start, operator_id="seed-s3-op1"
    )
    ...
    run_start(
        db, second_op.id, started_at=second_op.planned_start, operator_id="seed-s3-op2"
    )
